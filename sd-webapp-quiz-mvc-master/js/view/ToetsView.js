import {ToetsEvent} from "../model/ToetsEvent.js"
export class ToetsView {
    constructor(toets) {
        this.toets = toets;
        this.toets.addEventListener(ToetsEvent.CHANGED, this.onQuestionsChanged);
    }
    ...
    showNav(currentQuestion, questions) {
        ...
        questions.forEach(vraag => {
            if (vraag.getGivenAnswer() !== -1) {
                this.getElement('#no' + vraag.getQuestionID()).style['background-color'] = 'deepskyblue';
            }
            else {
                this.getElement('#no' + vraag.getQuestionID()).style['background-color'] = 'white';
            }
        });
    ...
    showResult() {
        let questions = this.toets.getQuestions();
